		...
		where TItemsViewSource : IGroupedItemsViewSource
	{
		...
		public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
		{
			...
			if (viewType == ItemViewType.GroupHeader)
			{
				...
				return new TemplatedItemViewHolder(itemContentView, ItemsView.GroupHeaderTemplate);
			}

			if (viewType == ItemViewType.GroupFooter)
			{
				var itemContentView = new ItemContentView(context);
				return new TemplatedItemViewHolder(itemContentView, ItemsView.GroupFooterTemplate);
			}
		...
		{
			if (holder is TemplatedItemViewHolder templatedItemViewHolder && (ItemsSource.IsGroupFooter(position) || ItemsSource.IsGroupHeader(position)))
			{
